...
interface CatererHeroProps {
  onSearch: (cuisine: string, city: string) => void
}

export function CatererHero({ onSearch }: CatererHeroProps) {
  const [cuisine, setCuisine] = useState("")
  const [city, setCity] = useState("")

  const handleSearchSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    onSearch(cuisine, city)
  }

  const QUICK_TAGS = ["Traditional Shadi", "Live BBQ", "Continental Buffet", "High Tea", "Dessert Bar"]
    ...
    <section className="relative bg-white border-b border-outline overflow-hidden py-12 md:py-20">
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="max-w-3xl">
          ...
          >
            <ShieldCheck className="w-4 h-4" /> Certified Taste & Sanitation
          </motion.div>
          ...
          >
            Pakistan's Premier Event & Wedding Caterers.
          </motion.h1>
          ...
          >
            Book verified shadi catering houses, high-tea caterers, live bbq stations, and custom dessert chefs. Protected by NEXUS tasting guarantees.
          </motion.p>
          ...
          >
            <div className="flex-1 flex items-center relative group px-2">
              ...
              <input
                type="text"
                placeholder="What cuisine style do you need? e.g. BBQ..."
                value={cuisine}
            ...
            >
              <Sparkles className="w-4 h-4" /> Find Caterers
            </button>
          </motion.form>
