...
export default function AdminThemeEditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [chatHistory, setChatHistory] = useState<{sender: 'user' | 'ai', text: string}[]>([
    { sender: 'ai', text: "Welcome back! Use the controls above to pick a starting layout and palette, or just type your instructions here." }
  ])
  ...
  useEffect(() => {
    params.then(resolvedParams => {
      fetch(`/api/admin/themes/${resolvedParams.id}`)
        .then(res => res.json())
        .then(data => {
          if (data.theme) {
            setForm({
              ...
              price: data.theme.price || "Free",
              type: data.theme.type || "free",
              preview_image: data.theme.preview_image || ""
            })
            if (data.theme.config) {
              setConfig(data.theme.config)
              if (data.theme.config.layoutType) setLayoutOption(data.theme.config.layoutType)
              if (data.theme.config.category) setCategory(data.theme.config.category)
            }
          } else {
  ...
  const handleAIGenerate = async () => {
    ...
    try {
      const promptContext = `[Context: Designing for category '${category}' using layout '${layoutOption}']\n${userText}`
  ...
  if (fetching) {
    return <div className="h-screen flex items-center justify-center bg-slate-100 font-bold text-slate-400">Loading Theme Machine...</div>
  }
