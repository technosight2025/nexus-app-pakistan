...
import { getQuotation, updateQuotation, DEFAULT_TERMS, DEFAULT_PAYMENT, DEFAULT_DELIVERABLES } from "@/lib/mock-db"
...
export default function EditQuotationPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    if (existing) {
      ...
      setPaymentSteps(existing.paymentSteps || DEFAULT_PAYMENT)
      setDeliverables(existing.deliverables || DEFAULT_DELIVERABLES)
      
      // Reverse calculate tax rate
      const subtotal = (existing.items || []).reduce((s: number, i: any) => s + (i.price * i.qty), 0)
      const afterDiscount = subtotal - (existing.discount || 0)
      if (afterDiscount > 0 && existing.tax > 0) {
        setTaxRate(Math.round((existing.tax / afterDiscount) * 100))
      }
    }
  }, [id])
  ...
  const handleSaveAndSend = () => {
    ...
    router.push(`/studio/quotations/${id}`)
  }
    ...
    <div className="space-y-5 md:space-y-6 max-w-6xl mx-auto pb-24">
      ...
      <div className="bg-white dark:bg-[#111118] border border-[#E5E7EB] dark:border-white/8 rounded-2xl p-4 md:p-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-[18px] md:text-[22px] font-black text-[#111827] dark:text-white leading-tight">Edit Quotation</h1>
            <p className="text-[11px] md:text-[12px] font-medium text-[#6B7280] dark:text-gray-400 mt-0.5">Modify your existing quotation package.</p>
          </div>
        </div>
